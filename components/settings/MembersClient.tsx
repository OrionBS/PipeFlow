...
import { UserMinus, Send, Loader2, X } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { inviteMember, removeMember, cancelInvite } from "@/actions/settings"
...
type Row = ActiveMember | PendingInvite
...
export function MembersClient({ rows: initialRows, workspaceId, plan, activeMemberCount }: MembersClientProps) {
  ...
  const atLimit = plan !== "pro" && activeMemberCount >= FREE_MEMBER_LIMIT
  ...
  function handleCancelInvite(id: string) {
    startTransition(async () => {
      const result = await cancelInvite(id, workspaceId)
      if (!result?.error) {
        setRows((prev) => prev.filter((r) => r.id !== id))
        router.refresh()
      }
          ...
          {rows.map((row) => (
            <div key={row.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
              <div
                ...
                style={{ background: avatarColor(row.initials), fontFamily: "var(--font-syne, sans-serif)" }}
              >
                {row.initials}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate" style={{ color: "#E8E8E8", fontFamily: "var(--font-dm-sans, sans-serif)" }}>
                  {row.kind === "member" ? row.name : row.email}
                </p>
                {row.kind === "member" && (
                  <p className="text-[11px] truncate" style={{ color: "#555559" }}>{row.email}</p>
                )}
              </div>

              {row.kind === "invite" && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: "rgba(251,191,36,0.1)", color: "#F59E0B", border: "1px solid rgba(251,191,36,0.2)", fontFamily: "var(--font-ibm-mono, monospace)" }}>
                  Pendente
                </span>
              )}

              <span
                className="text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0"
                style={{
                  background: row.role === "admin" ? "rgba(202,255,51,0.1)" : "rgba(255,255,255,0.05)",
                  color: row.role === "admin" ? "#CAFF33" : "#8A8A8F",
                  fontFamily: "var(--font-ibm-mono, monospace)",
                  border: row.role === "admin" ? "1px solid rgba(202,255,51,0.2)" : "1px solid rgba(255,255,255,0.06)",
                }}
              >
                {row.role === "admin" ? "Admin" : "Membro"}
              </span>

              {row.kind === "member" && row.role !== "admin" && (
                <RemoveButton onClick={() => handleRemoveMember(row.id)} disabled={isPending} icon="remove" />
              )}
              {row.kind === "invite" && (
                <RemoveButton onClick={() => handleCancelInvite(row.id)} disabled={isPending} icon="cancel" />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-3 max-w-sm">
          {atLimit && (
            <p className="text-[12px] px-3 py-2 rounded-lg" style={{ background: "rgba(239,68,68,0.08)", color: "#EF4444", border: "1px solid rgba(239,68,68,0.2)" }}>
              ...
              <a href="/settings/billing" style={{ color: "#CAFF33", textDecoration: "underline" }}>Fazer upgrade →</a>
            </p>
          )}
...
function RemoveButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="h-7 w-7 rounded-lg flex items-center justify-center transition-colors duration-150 disabled:opacity-40 shrink-0"
      ...
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLButtonElement
        el.style.color = "#555559"
        el.style.background = "transparent"
      }}
    >
      {icon === "remove" ? <UserMinus className="h-3.5 w-3.5" /> : <X className="h-3.5 w-3.5" />}
    </button>
  )
}
