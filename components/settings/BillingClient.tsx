...
import { startCheckout } from "@/actions/settings"
  ...
  return (
    <div className="grid gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-semibold tabular-nums" style={{ color, fontFamily: "var(--font-ibm-mono, monospace)" }}>
          {used}/{limit}
        </span>
      </div>
...
export function BillingClient({ plan, leadsUsed, membersUsed, leadLimit, memberLimit }: BillingClientProps) {
      ...
      <SettingsSection title="Plano Atual" description="Gerencie sua assinatura e limites de uso">
        <div className="flex flex-col gap-5">
          <div className="flex items-center gap-4">
            ...
            >
              <Zap className="h-5 w-5" style={{ color: isPro ? "#CAFF33" : "#555559" }} />
            </div>
            <div>
              <p className="text-base font-bold" style={{ color: "#E8E8E8", fontFamily: "var(--font-syne, sans-serif)" }}>
          ...
          {!isPro && (
            <div className="grid gap-3 p-4 rounded-lg" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <UsageBar used={leadsUsed} limit={leadLimit} label="Leads" />
              <UsageBar used={membersUsed} limit={memberLimit} label="Membros" />
            </div>
          )}
